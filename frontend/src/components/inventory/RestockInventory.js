...
export default function RestockInventory() {
    ...
    useEffect(() => {
        async function getDetails() {
            try {
                const result = await (await axios.get("https://hotel-management-3-t5ay.onrender.com/inventory/")).data.data
                console.log(result)
    ...
    async function updateRestock(dat) {
        try {
            ...
            const data = await (await axios.put(`https://hotel-management-3-t5ay.onrender.com/inventory/restock/${dat._id}`, newDetails)).status
            if (data === 200) {
                SoloAlert.alert({
                    title: "Welcome!",
